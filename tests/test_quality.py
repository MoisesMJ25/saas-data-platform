...
def spark() -> SparkSession:
    return (
        SparkSession.builder.appName("test-quality")
        .master("local[1]")
...
class TestRunSilverChecks:
    def test_clean_data_produces_no_critical(self, spark):
        ...
        critical_failed = [
            r for r in log_rows if r["check_severity"] == "critical" and not r["check_passed"]
        ]
    ...
    def test_null_precio_triggers_critical(self, spark):
        ...
        precio_check = next(r for r in log_rows if r["check_name"] == "precio_no_nulo_ni_negativo")
    ...
    def test_null_cantidad_normalizada_triggers_critical(self, spark):
        ...
        assert has_critical is True
        qty_check = next(r for r in log_rows if r["check_name"] == "cantidad_normalizada_positiva")
    ...
    def test_duplicate_key_warning_check(self, spark):
        ...
        dup_check = next(r for r in log_rows if r["check_name"] == "clave_negocio_sin_duplicados")
